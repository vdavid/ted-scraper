...
videos.slice(0, 10)
    .map(video => ({url: getURLForVideo(video), viewedCount: video.viewedCount, date: video.recordedOn}))
.forEach((data, index) => console.log(`${index + 1}. ${data.url} – Watched by ${data.viewedCount.toLocaleString()} people since ${data.date.split('T')[0]}`))
